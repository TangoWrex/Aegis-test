...
def threaded(connection: socket.socket, ipaddr: str, port: str):
    ...
    while True:
        try:
            ...
            # recieve id and pass from client
            id_pass = node.recv_timeout(connection)
            print(id_pass)

            # valid_user = node.valid_user()

            # If the node ID does not exist then the sensor cannot connect.

            # return error packet to the client
