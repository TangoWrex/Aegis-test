...
void send_struct(int socket, sensor_struct *sensor){
    ...
    if (!sensor){
        perror("sensor is null");
        return;
    }

    // send the sensor struct 
    printf("size of sensor datalen: %d", sensor->data_len);
    int end = 37;
    char sensor_struct_data[sensor->data_len + 1];
    // TODO: add "31" to identify the end of the string
    sprintf(sensor_struct_data, "%s%s%s%s%s%d", sensor->sensor_id, sensor->mac_addr, sensor->port, sensor->latitude, sensor->longitude, end);


    send(socket, sensor_struct_data, sensor->data_len + 1, 0);
}
