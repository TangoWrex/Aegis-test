...
from connection_with_queue_select import Connection
...
def service_connection(connection: Connection):
    ...
    Service a chat client socket connection.
        ...
        (2) Announce that it joined the chat by (producer) adding to queue
        so that consumer can publish
        (3) Receive chat messages and publish to active chatters by (producer)
        adding to queue so that consumer can publish
    """
